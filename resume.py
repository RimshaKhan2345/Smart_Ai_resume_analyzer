...
def get_ai_coaching(resume_text, job_description, match_score, 
                     missing_skills, recommendations):
    """
    Apna khud likha function jo already-computed analysis 
    ko AI se human-friendly coaching mein convert karta hai.
    """
    client = OpenAI(os.getenv("api_key"))
    
    system_prompt = """You are a career coach reviewing a candidate's 
resume-to-job match. You will receive a computed match score and 
skill gap data — NOT raw guessing. Your job is to translate this 
quantitative analysis into clear, encouraging, human-readable advice.
...
- For each missing skill, suggest ONE realistic way to close that gap 
  (course, project idea, or how to reframe existing experience)
- End with a short motivational note, no generic fluff
...
# ----- NLP Analysis Part:
def analyze_with_nlp(resume_text, job_description):
    """
...
def main():
    ...
    with btn_col:
        Ai_analyze = st.button("🤖 Analyze with AI ", use_container_width=True)
    if Ai_analyze:
        if not resume_text or not job_desc.strip():
            st.warning("⚠️ Please upload a resume AND paste a job description before analyzing with AI.")
            return




        analyze = st.button("🔍 Analyze with NLP ", use_container_width=True)
    ...
    if pdf_file:
        ...
        with st.spinner("Analyzing your resume..."):
            ...
            ai_result = analyze_with_nlp(resume_text, job_desc)
